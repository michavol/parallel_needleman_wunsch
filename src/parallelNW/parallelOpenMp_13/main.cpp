...
int main(int argc, const char *argv[])
{
   ...
   u_int64_t score_matrix_size = m*n;
 
   NW_SEQ_OPT::score_mat_t score_matrix(score_matrix_size);
   ...
   for(unsigned run = 0; run < runs+5; ++run){
      ...
      //initalise ScoreMatrix
        ...
        NW_SEQ_OPT::initialiseScoreMatrix(score_matrix, m, n, init_gap_length);
        ...
        for(; diag < n; ++diag){
            ...
            #pragma omp parallel for shared(score_matrix) num_threads(n_threads)
            ...
            for(u_int64_t diag_elem = 0; diag_elem <= diag; ++diag_elem){
                  ...
                  if(row > 0 && col > 0){
                     //cell up -> gap penalty
                     const NW_SEQ_OPT::len_t gap_up = score_matrix[idx_up].gapLength + 1;
                     const NW_SEQ_OPT::score_t score_up = score_matrix[idx_up].score + NW_SEQ_OPT::gapPenalty(gap_up);

                     //cell diagonal -> similarity function
                     const NW_SEQ_OPT::score_t score_diag = score_matrix[idx_diag].score + NW_SEQ_OPT::similarity(seq1[row-1],seq2[col-1]);

                     //cell left -> gap penalty
                     const NW_SEQ_OPT::len_t gap_left = score_matrix[idx_left].gapLength + 1;
                     const NW_SEQ_OPT::score_t score_left = score_matrix[idx_left].score + NW_SEQ_OPT::gapPenalty(gap_left);
                     ...
                     //current cell
                     score_matrix[i] = {max_score,gapLength,pred};
                  }
   ...
   NW_SEQ_OPT::score_t opt_score = NW_SEQ_OPT::backtrackSolution(score_matrix, seq1, seq2, aligned_seq1, aligned_seq2);
