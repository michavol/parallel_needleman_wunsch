...
using namespace NW_PAR;
 ...
 int main(int argc, char const *argv[])
 {
    ...
    if (local_rank == head) {
        ...
        row_above = score_line_t(num_cols);
        ...
        fillRowWithGaps(row_above, 0, num_cols, 0);
    ...
    score_matrix = score_mat_t(num_rows, score_line_t(num_cols));
    ...
    initialiseScoreMatrix(score_matrix, num_rows, num_cols, global_row_index + 1);
    ...
    for (unsigned i = 0; i < num_blocks; ++i) {
        ...
        if (local_rank != head) {
            recieveSegmentOfRow(row_above, current_col_index - 1, block_length + 1, i,
        ...
        computeScoreBlock(short_sequence, long_sequence,
                row_above, score_matrix, num_rows, current_col_index, block_length);
        ...
        if (local_rank < comm_size - 1) {
            sendSegmentOfRow(score_matrix, num_rows - 1, current_col_index - 1,
    ...
    if (num_extra_cols > 0) {
        ...
        // setup row above
        if (local_rank != head) {
            recieveSegmentOfRow(row_above,current_col_index - 1, num_extra_cols + 1, num_blocks,
                    communicator, local_rank - 1, local_rank);
        }
        ...
        // compute block of score matrix
        computeScoreBlock(short_sequence, long_sequence,
                row_above, score_matrix, num_rows, current_col_index, num_extra_cols);
